    ...
    <>
      <div className="grid grid-cols-5 h-screen text-center">
        <div className="col-span-1 bg-zinc-800">
          <h1 className="text-xl text-white pt-3 flex text-center justify-center">
          ...
          <ul className="text-left overflow-auto  mt-2">
            {Array.isArray(recentHistory) &&
              recentHistory.map((item, index) => (
                <li
                  className=" px-5 pl-5 text-zinc-400 cursor-pointer hover:bg-zinc-700 hover:text-zinc-200 truncate"
                  key={index} // Use index if there is no unique identifier
                >
                  {item}
                </li>
              ))}
        ...
        <div className="col-span-4 p-10">
                ...
                {result.map((item, index) => (
                  ...
                  >
                    {item.type == "q" ? (
                      <li
                        key={index + Math.random()}
                        className="text-right p-1 border  border-6 bg-zinc-700 border-zinc-700 rounded-tl-3xl  rounded-br-3xl rounded-bl-3xl w-fit"
                      >
                        <Answers
                          ans={item.text}
                          totalResult={1}
          ...
          <div className="bg-zinc-800 w-1/2 m-auto  text-white rounded-2xl flex border  border-zinc-700 p-1 h-16 pr-5 fixed bottom-5 left-1/2 transform -translate-x-1/2">
            <input
              type="text"
